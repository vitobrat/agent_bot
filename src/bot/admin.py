...
from src.aiopgsqldatabase.database import get_all_users, count_users, get_all_users_id
...
async def all_users():
    return "\n".join([f"{i}) ID - {user[0]}; Full name - {user[1]}; User name - {user[2]};"
                      f"Is admin - {user[3]}"for i, user in enumerate(await get_all_users())])
...
async def admin_newsletter_step_2(message: types.Message, state: FSMContext):
    users_id = [id[0] for id in await get_all_users_id()]
