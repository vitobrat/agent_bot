...
from src.pgsqldatabase.queries import (
    CREATE_TABLE_QUERY,
    DROP_TABLE_QUERY,
    COUNT_USERS_QUERY,
    DELETE_USER_QUERY,
...
class Database:
    ...
    async def print_all_users(self):
        ...
        return "\n".join(
            [
                f"{i}) ID - {user[0]}; Full name - {user[1]}; Username - {user[2]};Is admin - {user[3]};"
                f" History - {bool(user[4])}"
                for i, user in enumerate(await self.get_all_users())
            ]
        )
    ...
    async def add_user(
        ...
        if not isinstance(user_id, int):
            raise ValueError(
                f"Attempt to write in database incorrect user: {user_id}|{user_fullname}|{user_username}|"
                f"{is_admin}|{history}"
        ...
        if check_user is None:
            await conn.execute(
                INSERT_USER_QUERY.format(self.table_name),
                user_id,
                user_fullname,
                user_username,
                is_admin,
                json.dumps(history, cls=MessageEncoder),
            )
        await conn.close()
    ...
    async def get_user_history(self, user_id: int) -> list:
        ...
        row = await conn.fetchrow(
            GET_USER_HISTORY_QUERY.format(self.table_name), user_id
        )
        await conn.close()
    ...
    async def update_user_history(self, user_id: int, new_history=None) -> None:
        ...
        conn = await asyncpg.connect(**database)
        await conn.fetchrow(
            UPDATE_USER_HISTORY_QUERY.format(self.table_name),
            user_id,
